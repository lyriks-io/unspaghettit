...
const invariantInputSchema = z
  ...
  .describe('{ name, condition: { left: statePath, operator, right? }, message, description }. Semantics are inverted from rules: the condition must be TRUE for the invariant to HOLD. A FALSE condition is the violation, and a violation blocks the run. Phrase the condition as the always-true property (e.g. "amount greater_than 0", not "amount equals -1"). Never use a tautologically-false condition like equals to an impossible literal, it would block every action.');
