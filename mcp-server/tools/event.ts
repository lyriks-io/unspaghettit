...
export const registerEventTools = (deps: ToolDeps): void => {
  ...
  server.registerTool(
    ...
    {
      description:
        'Register a first-class event with optional payloadSchema. Action.emittedEvents and emit_event effects reference events by name; this entry lets the dashboard show payload shape and lets the validator catch unknown event names. Name must be dot-separated (e.g. "cart.item.added") and unique within the feature.',
      inputSchema: {
        featureId: z.string(),
        name: z.string().min(1),
        description: z.string().min(1),
    ...
    async ({ featureId, name, description, payloadSchema }) => {
  ...
  server.registerTool(
    ...
    async ({ featureId, eventId, name, description, payloadSchema }) =>
      runMutation(deps, {
        featureId: asFeatureId(featureId),
        transform: (exp) => {
