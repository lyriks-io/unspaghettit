...
describe('MCP server', () => {
  ...
  describe('get_spec_gaps', () => {
    ...
    it('flags a triggeredByEvent handler whose event no action in the feature emits', async () => {
      const { client, server } = await setup();
      const created = parseTextContent(
        await client.callTool({
          name: 'create_feature',
          arguments: { name: 'Dead handler', description: 'Feature used to verify dead-handler detection.' }
        })
      ) as { id: string };
      const surface = parseTextContent(
        await client.callTool({
          ...
          arguments: {
            ...
            name: 'Main',
            type: 'screen',
            description: 'Main screen for the dead-handler test.'
          }
        })
      ) as { id: string };
      await client.callTool({
        ...
        arguments: {
          ...
          type: 'boolean',
          defaultValue: false,
          description: 'Whether the screen is ready.'
        }
      });
      const action = parseTextContent(
        await client.callTool({
          ...
          arguments: {
            ...
            surfaceId: surface.id,
            name: 'On ghost',
            intent: 'React to an event nothing emits.',
            triggeredByEvent: 'ghost.event'
          }
        })
      ) as { id: string };
      // An effect keeps the unrelated "no effects" critical gap from firing.
      await client.callTool({
        name: 'add_effect',
        arguments: {
          featureId: created.id,
          ...
          effect: {
            ...
            path: 'main.ready',
            value: true,
            description: 'Marks the screen ready.'
          }
        }
      });

      const gaps = await callGaps(client, created.id);
      const deadHandler = gaps.filter(
        (g) => g.entityId === action.id && /handles event "ghost\.event"/.test(g.reason)
      );
      expect(deadHandler).toHaveLength(1);
      expect(deadHandler[0]!.severity).toBe('recommended');
      await server.close();
    });
  });
