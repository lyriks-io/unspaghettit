...
Effects are discriminated by type: set_state{path,value} | show_message{message,tone?} | emit_event{event} | block_action{reason} | allow_action | transition_surface{target}. set_state.value also accepts an Expression. Value:{kind:"add", left:{kind:"state", path:"player.vx"}, right:{kind:"param", name:"ax"}} writes vx+ax atomically. In apply_batch the transition_surface effect accepts targetRef so it can wire to a surface created earlier in the same batch. An Action names who fires it with actor: user (default) | system | schedule | event (absent reads as event when triggeredByEvent is set); a non-user action is not asked what a person sees when it is blocked, and a person-fired action that is silent when blocked says so with an onBlocked effect no_feedback{reason} instead of an invented message.
