...
. \`presentation:true\` marks a view-only/presentation surface EXCLUDED from behavior-maturity scoring (rendered and navigable, but its empty body never drags the feature rollup). Do not infer it from type:"screen".
  add_action           { ref?, surfaceRef|surfaceId, name, intent, requiredStates?, emittedEvents?, bypassInvariants?, invariantRelaxation?, triggeredByEvent?, actor?, evolution? }
...
. \`actor: "user"|"system"|"schedule"|"event"\` says who fires the action. Absent means \`user\`, or \`event\` when \`triggeredByEvent\` is set; every read shows the effective value. Set it when no person fires the action (a sensor reading, a timer, a host callback): the blocked-run maturity check asks what a PERSON sees when the action refuses, and is waived for a non-user actor, so do not invent a show_message the product never displays. \`actor:"event"\` does not require \`triggeredByEvent\` (the host product may fire the action from an event this model does not describe). On update_action pass \`actor:null\` to clear it back to the default.
...
. \`type\` is string | number | boolean | enum | object | array. Common synonyms are folded to the canonical type: int/integer → number (so a counter with type:"int", defaultValue:0 is fine), bool → boolean, str/text → string, list/collection → array, map/dict → object. There is NO \`list\` type: lists are \`type:"array"\`, and the append_to_list / remove_from_list / update_list_item effects operate on array-typed paths.
...
. Same effect.type enum as above (all 11), plus \`no_feedback{reason,description}\`, which is valid ONLY with \`onBlocked:true\` (refused as a success effect, a rule effect or an outcome effect). It states that a blocked attempt shows nothing, on purpose, and \`reason\` says why; the simulator treats it as a no-op. It satisfies the blocked-run maturity check for any actor: use it for a person-fired action that is silent when refused, instead of a message that does not exist in the product. transition_surface accepts \`targetRef\` for surfaces minted earlier in the same batch. \`onBlocked:true\` appends to the action's onBlockedEffects (the fallback the simulator runs when the action is BLOCKED - state writes suppressed, but transition_surface/emit_event/show_message still fire) instead of the success-path effects. update_effect / remove_effect take the same \`onBlocked\` flag to target that list.
...
. NEST the goal body under \`goal\` - its own \`kind\` ("reachable" | "always_reachable") collides with the op-kind discriminator \`kind:"add_reachability_goal"\`, and a JSON object cannot carry two \`kind\` keys, so a flat \`{ kind:"add_reachability_goal", ..., kind:"reachable" }\` silently loses the op kind. (Same collision pattern as add_resource / add_dependency.) Feature-level LIVENESS goal, the complement to invariants (safety). \`goal.kind:"reachable"\` = some reachable state satisfies \`condition\` (the target is achievable at all); \`goal.kind:"always_reachable"\` = the target stays reachable from EVERY reachable state (no trap). \`condition\` is the target state, same shape as an invariant condition; no parameter scope. Evaluated by model_check / verify over the reachable state space (bounded), which return the shortest action path to a trap on an always_reachable failure.
  add_acceptance_criterion { ref?, title, given?, when?, then?, expectedOutcome?, relatedSurfaceId?, description?, status?, relations? }
...
. \`status: "active"|"superseded"|"draft"\` says where the criterion stands (absent = active; a draft supersedes nothing yet). \`relations: [{ kind: "supersedes"|"refines"|"exception_to", criterionId|criterionRef, featureId?, note? }]\` says what it replaces, narrows or carves a case out of. \`criterionId\` must be a criterion of THIS feature unless \`featureId\` names another feature (then it is carried unresolved, like relatedSurfaceId); no self relation; no duplicate (kind, criterionId). \`criterionRef\` points at a criterion added earlier in the same batch, and update_acceptance_criterion accepts \`criterionRef\` in place of \`criterionId\`, so one batch can add the successor and mark the old one superseded. On update, \`relations\` is a full replacement, \`relations:[]\` clears them and \`status:null\` clears the status. Statuses are NEVER set for you: every read computes \`supersededBy\` and a one-line \`standing\` (active | superseded by <ids> | superseded (no successor named) | draft | active, but superseded by <ids>), and a criterion that another one supersedes while it is still active comes back as a \`warnings\` entry in the answer (apply_batch and the granular tools) and as a recommended gap in get_spec_gaps. None of this affects maturity or any verification score. A criterion is versioned like any element (\`criterion:<id>\`), so an index entry that maps what verifies it goes stale in get_drift when its wording, status or relations change.
...
. \`presentation:true\` excludes the surface from behavior-maturity scoring; \`presentation:false\` re-includes it.
  update_action           { surfaceRef|surfaceId, actionRef|actionId, name?, intent?, requiredStates?, emittedEvents?, actor? }
...
. Same kind-collision rule as add_resource.
  update_dependency           { dependencyId, name?, dependencyKind?, description?, provider?, operations?, assumptions? }
. \`operations\` is a full replacement (fresh ids minted).

Every update op also accepts its editable fields under \`patch:{...}\` instead of flat; both spellings are equivalent. An update whose target id does not resolve, or whose patch is empty, is an ERROR - updates never no-op silently.
