...
| `get_implementation_status` | Detailed sidecar for one feature (or filtered to one action/surface). Shows captured fields, locations, staleness flags, and `auditMeta.verifiedAt` on an action proven against the code. The whole-feature read also answers the evidence the last syncs KEPT: `criteria[]` (every acceptance criterion with `state` verified / failing / unverified / none, and `stale` when the criterion changed after the `specVersion` its check was written against), `orphanedCriteria`, and `verified: { actions, total }`. |
