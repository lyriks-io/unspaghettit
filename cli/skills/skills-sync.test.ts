...
/**
 * cli/skills/ is the canonical home for the three skills that ship via
 * `unspa init`. .claude/skills/ holds the same three so they apply when
 ...
 * three ever diverge again. Fix by editing the cli/skills/ copy and
 * running `npm run skills:sync`.
 */
const __dirname = dirname(fileURLToPath(import.meta.url));
const ROOT = join(__dirname, '..', '..');
const SHARED = ['unspa-audit', 'unspa-edit', 'unspa-implement'] as const;
