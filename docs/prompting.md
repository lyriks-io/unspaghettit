...
2. It reads the code and models it batch by batch - one Feature per coherent flow. Each source file it read is stored via `attach_source_path` (the MCP server reads the file from disk itself, so the content never travels through the conversation twice) and **every extracted element is traced to the exact code span it came from** (`record_element_spans`, batched). `finalize_analysis` refuses to lock until nothing is untraced, so the model can't contain invented behavior.
...
4. **Account for the sources honestly (optional, for a defensible extraction).** When code and docs disagree, rank the sources (`authority`/`artifact` on attach, or `classify_source`) and record the disagreement with `flag_conflict` rather than silently modeling one side - the higher-authority claim is the suggested winner. To prove you didn't miss anything, `stage_candidate` each behavior a source describes and `dispose_candidate` it (accepted / rejected / merged / ...); `get_source_coverage` then reports the share of each source that reached the model and lists what's still unresolved - the answer to "what might we be missing?".
5. **Find what's shallow or missing.** `score_feature`, `get_spec_gaps`, and `get_implementation_gaps` show where the model under-describes the code (or the code is unindexed). Iterate until it audits clean.
6. You now have a legible, machine-checkable map of what your product does, wired to the source - so future drift is visible.
