...
| --- | --- | --- |
| **Default** | `unspa dashboard` | Loopback bind (`127.0.0.1:3000`). No auth, no telemetry, no callhome. Single-machine trust boundary, with always-on DNS-rebinding/CSRF hardening (`Host` + write-`Origin` validation) so a page in your browser can't drive the local API. |
