...
Provenance proves *attribution* (every element came from somewhere); a second, optional layer proves *completeness* (every important source behavior is accounted for) and settles disagreements by evidence rather than reading order. Rank each source by `authority` (normative / supporting / observed / unknown) and `artifact` (contract, implementation, test, documentation, interview), so an unstated authority is derived from the artifact and a later contradiction resolves by weight. When two sources disagree about the same behavior, `flag_conflict` records it (the higher-authority claim is the suggested winner; a tie is flagged as genuine ambiguity) instead of silently picking one, and `resolve_conflict` closes it with a written reason. `stage_candidate` holds a behavior a source proposes *before* it commits to the model, each with a review disposition (accepted / merged / rejected / out_of_scope / conflict), and `get_source_coverage` then reports the share of a source's behavior that reached the model versus what stays excluded or unresolved - the answer to "what might we be missing?".
...
- **Source trust & extraction completeness** - rank sources by authority, record contradictions between them as first-class conflicts (resolved by authority), stage each proposed behavior with a review disposition, and measure the share of a source that actually reached the model (`get_source_coverage`).
