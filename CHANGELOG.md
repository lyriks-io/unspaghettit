...
- **Verify view in the dashboard.** A new **Verify** tab on the feature page (`/features/<id>/verify`, beside Graph) that runs the whole verification spine server-side and renders the per-feature verdict: every check `pass` / `warn` / `fail` with its detail, and invariant / liveness **counterexamples shown as their shortest action path** — the model checker's most useful output finally has a human surface. A model-check toggle and re-run button; scoped to the feature's project so cross-feature checks resolve, with sibling verdicts tucked under a "Project context" disclosure. Backed by a new `/api/snapshots/<id>/verify` endpoint over the same `VerifyFeatures` use case the CLI and MCP use, so the dashboard, `unspa check`, and `verify` all agree.
