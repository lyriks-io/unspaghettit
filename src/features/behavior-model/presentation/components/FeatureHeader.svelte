...
<header class="border-b border-slate-200 pb-4">
  <div class="flex flex-wrap items-start justify-between gap-3">
      ...
      {:else}
        ...
        <div class="flex flex-wrap items-center gap-3 text-xs text-slate-500">
          <button
            ...
            onclick={() => (editing = true)}>Edit metadata</button
          >
    ...
    <div
      class="flex shrink-0 flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white p-1.5 shadow-sm shadow-slate-950/5"
    >
      <div class="flex rounded-lg border border-hairline bg-white p-1">
        ...
        <button
          ...
          class="rounded-md px-2 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50 {historyOpen
            ? 'bg-slate-100'
            : ''}"
