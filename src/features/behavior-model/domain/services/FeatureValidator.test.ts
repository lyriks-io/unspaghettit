...
describe('validateReferenceIntegrity', () => {
  ...
  it('does not strand a feature that already stores an out-of-domain enum write', () => {
    // The write gate is diff-aware: a pre-existing bad value must stay editable
    // (the error is in the baseline too), or shipping this check would make
    // every feature authored through the lax path unwritable.
    const stored = planFeature({ effectValue: 'family' });
