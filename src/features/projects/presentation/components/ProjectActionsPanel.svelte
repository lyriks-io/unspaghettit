      ...
      <table class="w-full text-sm">
        <thead class="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
          <tr>
            ...
            <th class="px-3 py-2">Definition stats</th>
            <th class="px-3 py-2">Shared references</th>
          </tr>
        </thead>
        <tbody>
          {#each actions as action (action.actionId)}
            <tr class="border-t border-slate-100 align-top">
              ...
              <td class="px-3 py-2 text-xs text-slate-600">
                <a href={`/features/${action.featureId}`} class="text-brand-700 hover:underline"
                  >{action.featureName}</a
                >
                <div>{action.surfaceName}</div>
              </td>
              <td class="px-3 py-2 text-xs tabular-nums text-slate-600">
                ...
                <div>{action.counts.effects} effects · {action.counts.invariants} invariants</div>
                <div>{action.counts.scenarios} scenarios · {action.counts.outcomes} outcomes</div>
              </td>
